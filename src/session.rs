...
impl ConnectionSession<Ready> {
    ...
    pub fn apply_subscription_message(&mut self, mid: u16, revision: MidRevision) {
        match mid {
            ...
            900 => self.state.subscriptions.subscribe_trace_curve(2, revision),
            9 => self.state.subscriptions.unsubscribe_trace_curve(),
            _ => {}
        }
    }
    ...
    /// Subscribe to relay function events (MID 216)
